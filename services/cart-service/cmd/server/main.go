...
import (
	...
	"github.com/ecommerce/cart-service/internal/api"
	"github.com/ecommerce/cart-service/internal/config"
	"github.com/ecommerce/cart-service/internal/messaging"
...
func main() {
	...
	// Initialize service
	cartService := service.NewCartService(cartRepo, kafkaWriter, log)

	// Initialize Kafka consumer
	consumer := messaging.NewEventConsumer(cfg.Kafka.Brokers, cfg.Kafka.GroupID, cartService, log)
	consumer.Start(context.Background())
	defer consumer.Stop()

	// Initialize handler
	handler := api.NewCartHandler(cartService, log)
	...
	go func() {
		log.Infof("Cart Service listening on port %s", cfg.Server.Port)
