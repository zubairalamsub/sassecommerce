...
import (
	...
	"github.com/ecommerce/recommendation-service/internal/api"
	"github.com/ecommerce/recommendation-service/internal/config"
	"github.com/ecommerce/recommendation-service/internal/messaging"
...
func main() {
	...
	// Initialize service
	recService := service.NewRecommendationService(recRepo, kafkaWriter, log)

	// Initialize Kafka consumer
	consumer := messaging.NewEventConsumer(cfg.Kafka.Brokers, cfg.Kafka.GroupID, recService, log)
	consumer.Start(context.Background())
	defer consumer.Stop()

	// Initialize handler
	handler := api.NewRecommendationHandler(recService, log)
	...
	go func() {
		log.Infof("Recommendation Service listening on port %s", cfg.Server.Port)
