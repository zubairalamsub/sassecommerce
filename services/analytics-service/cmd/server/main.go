...
import (
	...
	"github.com/ecommerce/analytics-service/internal/api"
	"github.com/ecommerce/analytics-service/internal/config"
	"github.com/ecommerce/analytics-service/internal/messaging"
	...
	"github.com/ecommerce/analytics-service/pkg/logger"
...
func main() {
	...
	// Initialize service
	analyticsService := service.NewAnalyticsService(analyticsRepo, kafkaWriter, log)

	// Initialize Kafka consumer
	consumer := messaging.NewEventConsumer(cfg.Kafka.Brokers, cfg.Kafka.GroupID, analyticsService, log)
	consumer.Start(context.Background())
	defer consumer.Stop()

	// Initialize handler
	handler := api.NewAnalyticsHandler(analyticsService, log)
	...
	router.Use(metrics.Middleware("analytics-service"))
	...
	go func() {
		log.Infof("Analytics Service listening on port %s", cfg.Server.Port)
