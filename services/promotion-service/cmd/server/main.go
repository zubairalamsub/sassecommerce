...
import (
	...
	"github.com/ecommerce/promotion-service/internal/api"
	"github.com/ecommerce/promotion-service/internal/config"
	"github.com/ecommerce/promotion-service/internal/messaging"
	...
	"github.com/ecommerce/promotion-service/pkg/logger"
...
func main() {
	...
	log.Info("Starting Promotion & Discount Service...")
	...
	// Initialize service
	promotionService := service.NewPromotionService(promotionRepo, kafkaWriter, log)

	// Initialize Kafka consumer
	consumer := messaging.NewEventConsumer(cfg.Kafka.Brokers, cfg.Kafka.GroupID, promotionService, log)
	consumer.Start(context.Background())
	defer consumer.Stop()

	// Initialize handler
	handler := api.NewPromotionHandler(promotionService, log)
	...
	router.Use(metrics.Middleware("promotion-service"))
	...
	go func() {
		log.Infof("Promotion Service listening on port %s", cfg.Server.Port)
